...
async function handleLoanSubmit(e) {
    ...
    // Calculate initial balance by deducting past EMIs
    ...
    if (startDateStr) {
        ...
        // Example: Start Aug 15, EMI Date 1st. First EMI is Sept 1st.
        // Example: Start Aug 1, EMI Date 1st. First EMI is Aug 1st.
        if (iterDate < start) {
    ...
    const data = {
        ...
        interestRate: parseFloat(document.getElementById('loanROI').value) || 0,
        tenureMonths: parseInt(document.getElementById('loanTenure').value) || 0,
        emiAmount: emiAmount,
        emiDate: emiDate,
        startDate: startDateStr,
        remainingBalance: remainingBalance, // Adjusted balance
    ...
    try {
        ...
        if (paidEmiCount > 0) {
            showToast(`Loan created. Adjusted for ${paidEmiCount} past EMIs.`);
        } else {
...
function updateDashboard() {
    ...
    loans.forEach(l => {
        if (l.status === 'active') totalOutstandingLoan += l.remainingBalance;
    });
    ...
    // = Income - (Expenses + EMIs + Investments)
    ...
    const balance = totalIncome - totalExpense - totalEMI - totalInvestedOutflow;

    // Animate numbers
    animateValue('totalIncome', totalIncome);
    animateValue('totalExpenses', totalExpense + totalEMI); // Show combined expenses
    animateValue('totalPending', totalPending + totalOutstandingLoan); // Add outstanding loans here
    animateValue('totalBalance', balance);
    
    // Update Breakdown Text
    const breakdownEl = document.getElementById('balanceBreakdown');
    if (breakdownEl) {
        let bText = `Inc: ${formatCompact(totalIncome)} - Exp: ${formatCompact(totalExpense + totalEMI)}`;
        ...
        return `
            <div class="list-group-item p-3 border-bottom">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <span class="badge bg-primary">EMI: ${formatCurrency(l.emiAmount)}</span>
                </div>
                <div class="d-flex justify-content-between small text-muted mb-1">
                    <span>Balance: ${formatCurrency(l.remainingBalance)}</span>
                    <span>Total: ${formatCurrency(l.totalAmount)}</span>
                </div>
                ...
                <div class="d-flex justify-content-between mt-2 align-items-center">
                    <small class="text-muted">
                        <i class="bi bi-calendar-event"></i> Due: ${l.emiDate}th
                        ${l.interestRate ? `<span class="ms-2">• ROI: ${l.interestRate}%</span>` : ''}
                        ${l.tenureMonths ? `<span class="ms-2">• ${l.tenureMonths}m</span>` : ''}
                    </small>
                    <button class="btn btn-link btn-sm text-danger p-0 text-decoration-none" onclick="deleteLoan('${l.id}')">Delete</button>
                </div>
            </div>
        `;
    }).join('');
...
function renderInvestments(list) {
    ...
    container.innerHTML = list.map(i => {
        ...
        const profitPercent = ((profit / i.investedAmount) * 100).toFixed(1);
        ...
        return `
            <div class="list-group-item p-3 border-bottom">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <button class="btn btn-sm btn-outline-secondary py-0 px-2" onclick="updateInvestmentValue('${i.id}', ${i.currentValue})">Update</button>
                </div>
                <div class="row g-2">
                    <div class="col-6">
    ...
    for (const loan of loans) {
        ...
        if (currentDay >= loan.emiDate && loan.lastEmiPaidMonth !== currentPeriod) {
            ...
            try {
                ...
                // 2. Update Loan
                const newBalance = loan.remainingBalance - loan.emiAmount;
